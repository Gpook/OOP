...
void menu(RenderWindow& window) {
	...
	menuTexture3.loadFromFile("images/close.png");
	...
	while (isMenu)
	{
		...
		{
			if (menuNum == 1) { window.close(); isMenu = false;  shouldClose = 0;  }
			
			if (menuNum == 3) { shouldClose = 1;  isMenu = false; 		exit(0);	}
		}
		window.draw(menuBg);
		window.draw(menu1);
		window.draw(menu3);
...
int main()
{
	...
	Animation rBullet(t8, 0, 0, 32, 64, 16, 0.8);
	...
	while (app.isOpen())
	{
			...
			if (event.type == Event::KeyPressed)
				...
				if (deadearly){
					if (event.key.code == Keyboard::Space)
					{
						...
						b->settings(rBullet, p->x, p->y, p->angle += rand() % 7 - 3, 10);
		...
		message.setString("Score:" + playerScoreString.str());
		message.setPosition(W / 2 - 150, H / 2 + 100);
		app.draw(message);
